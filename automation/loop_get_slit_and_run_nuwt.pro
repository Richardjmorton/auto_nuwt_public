;+
;NAME: LOOP_GET_SLIT_AND_RUN_NUWT
;
;PURPOSE:
;   [to write later]
;
...
PRO LOOP_GET_SLIT_AND_RUN_NUWT, timestamps=timestamps, img_locs=img_locs, $
                                regions=regions, $
                                slit_type=slit_type, $
...
FOR f=0, (num_files-1) DO BEGIN
    ...
    IF NOT KEYWORD_SET(rerun_nuwt) THEN RESTORE, filename=input_files[f]
    ...
    FOR slitnum=0, (num_slits_per_file-1) DO BEGIN
        ...
        IF NOT KEYWORD_SET(rerun_nuwt) THEN BEGIN
            IF STRLOWCASE(slit_type) EQ 'arc' THEN BEGIN
                ...
                NUWT_ARC_SLIT, sm_aia_data, sm_slit_td, index=aia_index, radius=current_slit_radius, units_radius='Mm', /rel_limb, $
                          ...
                          clockwise=slit_direction[f], /noopen
            ...
            save, slit_meta, sm_slit_td, us_slit_td, filename=save_folder+slit_filename
        ENDIF ELSE BEGIN
        ...
        RUN_NUWT, us_slit_td, /aia, slit_meta=slit_meta, grad=0.5, min_tlen=20, /gauss, /pad_fft, $
...
; FOREACH, FILE_TS, timestamps
;     ; restore, filename='/DATADRIVE1/data/SDO/processed/'+FILE_TS+'_aia_171_south_sm_and_us.sav'
;     ;
;     ; NUWT_ARC_SLIT, sm_aia_data, sm_south_ch_td, index=aia_index, radius=15.0, units='Mm', /rel_limb, start_ang=265, subtend=10, width=3, /noopen
;     ; NUWT_ARC_SLIT, us_aia_data, us_south_ch_td, index=aia_index, radius=15.0, units='Mm', /rel_limb, start_ang=265, subtend=10, width=3, /noopen, meta_out=south_ch_meta
;     ;
;     ; save, south_ch_meta, sm_south_ch_td, us_south_ch_td, filename='/DATADRIVE1/data/slits/'+FILE_TS+'_south_ch_15Mm_td.sav'
;     ;
;     ; PLOT_NUWT_SLIT_LOC, sm_aia_data, slit_meta=south_ch_meta, title=FILE_TS+', SDO / AIA 171, 15Mm', filetag=FILE_TS+'_15Mm'
;
;     restore, filename='/DATADRIVE1/data/slits/'+FILE_TS+'_south_ch_15Mm_td.sav'
;
;     RUN_NUWT, us_south_ch_td, /aia, slit_meta=south_ch_meta, grad=0.5, min_tlen=20, /gauss, /pad_fft, $
;                        default_plot_header=FILE_TS+', South CH, 15 Mm, Unsharp', filetag=FILE_TS+'_south_ch_15Mm_us'
;
;     ; RUN_NUWT, us_north_ch_td, /aia, slit_meta=north_ch_meta, grad=0.5, min_tlen=20, /gauss, /pad_fft, default_plot_header=plt_h, filetag=sv_name
; ENDFOREACH
